...
function normalizeZA(input: string) {
  ...
  if (trimmed.startsWith("+"))
    return "+" + trimmed.replace(/[^\d]/g, "").slice(0);
...
export default function SignupForm() {
  ...
  const verifySms = async () => {
    ...
    const res = await fetch("/api/verify-sms-otp", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ number, code: smsCode }),
    });
    const data = await res.json();
  ...
  const sendWa = async () => {
    ...
    if (data.ok) {
      setWaSent(true);
      if (data.devCode) setWaCode(data.devCode);
    }
  };
  const verifyWa = async () => {
    ...
    const res = await fetch("/api/verify-whatsapp-otp", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ number, code: waCode }),
    });
    const data = await res.json();
  ...
  const shareButtons = (link: string) => (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <a
        className={cn(
          "inline-flex items-center rounded-md border px-3 py-2 text-sm hover:bg-accent",
        )}
      ...
      <a
        ...
        href={`https://twitter.com/intent/tweet?text=${encodeURIComponent("I just joined DriveIQ! " + link)}`}
        target="_blank"
        rel="noreferrer"
      >
      ...
      <a
        ...
        href={`https://wa.me/?text=${encodeURIComponent("I just joined DriveIQ! " + link)}`}
        target="_blank"
        rel="noreferrer"
      >
      ...
      <a
        ...
        href={`https://tiktok.com/upload?url=${encodeURIComponent(link)}`}
        target="_blank"
        rel="noreferrer"
      >
    ...
    return (
      <div className="rounded-xl border bg-card/60 p-6 shadow space-y-4 text-center">
        ...
        <div className="text-sm text-muted-foreground">
          Share now (tag DriveIQ) to start earning credits.
        </div>
        {shareButtons(result.referralLink)}
        <div className="rounded-md border bg-background/60 p-3 text-xs text-muted-foreground">
          Earn {creditsPerReferral} credits ({randEquivalent}) per paid
          referral. Minimum withdrawal balance {minWithdrawal}. Redeemable as
        ...
        <div className="flex flex-wrap gap-2 pt-2 justify-center md:justify-start">
          ...
          <Button type="button" onClick={sendVision}>
            Send Vision Test Link
          </Button>
        </div>
        {typeof remaining === "number" && (
          <p className="text-xs text-muted-foreground">
            Remaining Early Bird slots:{" "}
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="rounded-xl border bg-card/60 p-6 shadow space-y-6 text-center md:text-left"
    >
      <div className="grid gap-3 sm:grid-cols-2">
        <button
          type="button"
          onClick={() => setSocialProvider("facebook")}
          className="flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-accent"
        >
          <Globe className="size-4" /> Continue with Facebook
        </button>
        <button
          type="button"
          ...
          className="flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-accent"
        >
          <Globe className="size-4" /> Continue with Instagram
        </button>
        <button
          type="button"
          onClick={() => setSocialProvider("tiktok")}
          className="flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-accent"
        >
          <Globe className="size-4" /> Continue with TikTok
        </button>
        <button
          type="button"
          ...
          className="flex items-center justify-center gap-2 rounded-md border px-3 py-2 text-sm hover:bg-accent"
        >
          <Globe className="size-4" /> Continue with X/Twitter
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <input
            className="mt-1 w-full rounded-md border bg-background px-3 py-2 text-sm outline-none focus:ring-2 ring-ring"
                ...
                <>
                  {!emailSent ? (
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        className="w-28 rounded-md border bg-background px-2 py-2 text-xs"
          ...
          <div className="mt-1 flex gap-2">
            ...
            {hasWhatsApp ? (
              ...
              <span className="text-xs text-muted-foreground">
                No WhatsApp? Email verification required.
              </span>
            )}
          </div>
